...
SetNewValue(G4UIcommand *command, G4String newValues) {
    ...
    if (command == DetGeometrySelectCmd) {
    ...
    } else if (command == DetGeometryOptCmd) {
        // read extra options.
        ...
        if (ss >> opt >> std::ws && std::getline(ss, optparams, '\0')) {
            ...
            G4cout << "/detGeometry/opt " << "Opt:" << opt << "optparams: " << optparams << G4endl
                << std::flush;
            G4cout << "    ... No option given" << G4endl << std::flush;
        }
    } else {
        G4cerr << "invalid detector \"set\" command: " << command->GetCommandPath() << command->
            GetCommandName() << "\n"
            << std::flush;
